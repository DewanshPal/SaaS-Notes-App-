...
export async function GET(req: NextRequest, { params }: RouteParams) {
    ...
    try {
        ...
        const note = await NoteModel.findOne({ _id: id, tenantId: tenant._id, userId: user._id });
        if (!note) {
...
export async function PUT(req: NextRequest, { params }: RouteParams) {
    ...
    try {
        ...
        const updatedNote = await NoteModel.findOneAndUpdate(
            { _id: id, tenantId: tenant._id, userId: user._id },
            { title, content },
            { new: true }
        );
...
export async function DELETE(req: NextRequest, { params }: RouteParams) {
    ...
    try {
        ...
        const deletedNote = await NoteModel.findOneAndDelete({ _id: id, tenantId: tenant._id, userId: user._id });
        if (!deletedNote) {
            return NextResponse.json({ success: false, message: "Note not found or not authorized" }, { status: 404 });
        }
